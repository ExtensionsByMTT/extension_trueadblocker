(function () {
    ...
    setInterval(function () {
      if (window._websockets?.[0] == undefined) return;
      window._websockets[0].hooks.beforeReceive = function (messageEvent) {
    ...
    async function manipulateStateMachine(
      stateMachine,
      startingStateIndex,
      isReplacingState
    ) {
      ...
      do {
        ...
        for (var i = 0; i < states.length; i++) {
          ...
          if (trackURI.includes(":ad:") && isAd(state) == true) {
            ...
            if (isAd(nextState)) {
              ...
              try {
                ...
                do {
                  var latestTrack =
                    futureStateMachine["tracks"][nextState["track"]];
                  futureStateMachine = await getStates(
                    futureStateMachine["state_machine_id"],
                    nextState["state_id"]
                  );
                ...
                if (isAd(nextState)) {
                  ...
                  console.error(
                    "could not find the next ad-free state. state machine was:"
                  );
                ...
                if (i == startingStateIndex && !isReplacingState) {
                  ...
                  console.log(
                    "SpotifyAdRemover: Removed ad at " +
                      trackURI +
                      ", more complex flow"
                  );
                }
                ...
                console.log(
                  "SpotifyAdRemover: Shortned ad at " +
                    trackURI +
                    " due to exception:"
                );
          ...
          if (
            i == startingStateIndex &&
            !isReplacingState &&
            tamperedStatesIds.includes(stateId)
          ) {
    ...
    async function getStates(stateMachineId, startingStateId) {
      var statesUrl =
        "https://spclient.wg.spotify.com/track-playback/v1/devices/" +
        deviceId +
        "/state";
      var body = {
        ...
        previous_position: 5504,
        debug_source: "resume",
      };
  
      var result = await originalFetch.call(window, statesUrl, {
        method: "PUT",
        headers: {
      ...
      if (
        ...
        await initalize();
        result = await originalFetch.call(window, statesUrl, {
          method: "PUT",
          headers: {
            Authorization: "Bearer " + accessToken,
            "Content-Type": "application/json",
    ...
    function* statesGenerator(
      states,
      startingStateIndex = 2,
      nextStateName = "skip_next"
    ) {
      ...
      for (
        ...
        state != undefined;
        state = states[state["transitions"][nextStateName]["state_index"]]
      ) {
        iterationCount++;
    ...
    function onAdRemoved(trackURI, skipped = false) {
      if (!removedAdsList.includes(trackURI)) {
        ...
        if (skipped) showToast("Skipped ad");
        else showToast("Removed ad");
    ...
    function onAdCouldntBeRemoved(trackURI) {
      console.log(
        "SpotifyAdRemover: Could not remove ad at " +
          trackURI +
          " because it is currently playing"
      );
    ...
    function checkInterception() {
      var isInterceptionWorking =
        isFetchInterceptionWorking && isWebScoketInterceptionWorking;
      console.log(
        "fetch",
        isFetchInterceptionWorking,
        "ws",
        isWebScoketInterceptionWorking
      );
